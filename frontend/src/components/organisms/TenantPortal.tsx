...
function usagePoints(invoices: readonly Invoice[], code: string): UsagePoint[] {
  return invoices
    ...
    .map((invoice) => ({
      ...
      usage: lineFor(invoice, code)!.quantity,
    }))
    .reverse();
}
...
export function TenantPortal() {
  ...
  const { overview, invoices, status, error } = useAppSelector(
    (state) => state[SLICE.tenant],
  );
  ...
  const summary = useMemo(() => {
    ...
    return {
      ...
      latest: points[points.length - 1]!.usage,
    };
  }, [points]);
  ...
  if (!overview) {
    return (
      <Notice message={error ?? STRINGS.tenant.noContract} tone="danger" />
    );
  }

  const seriesLabel =
    series === SERVICE_CODE.water
    ...
    <div className="stack">
      ...
      {selected ? (
        ...
        >
          ...
          <div className="invoice-total">
            ...
            <div>
              ...
              <strong className="num">
                {formatMoney(selected.paid_amount)}
              </strong>
            </div>
            <div>
              <div className="stat-label">
                {STRINGS.tenant.invoiceRemainingLabel}
              </div>
              <strong className="num">
                {formatMoney(
                  Math.max(selected.total - selected.paid_amount, 0),
                )}
              </strong>
            </div>
            <div>
          ...
          {/* Arrears belong to the contract, not to this invoice, so they are
              shown only on the period the tenant is being asked to pay now —
              on an older invoice the same figure would be counted twice, once
              here and once on the card at the top of the page. */}
          {balance.previous_due > 0 &&
          selected.period === balance.current_period ? (
            <div className="invoice-total">
              <div>
                <div className="stat-label">
                  {STRINGS.tenant.invoicePreviousDue}
                </div>
                <strong className="num">
                  {formatMoney(balance.previous_due)}
                </strong>
              </div>
              <div>
                <div className="stat-label">
                  {STRINGS.tenant.invoiceAmountDue}
                </div>
                <b className="num">
                  {formatMoney(
                    Math.max(selected.total - selected.paid_amount, 0) +
